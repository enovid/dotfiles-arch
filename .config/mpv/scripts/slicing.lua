...
local o = {
    target_dir = "C:/cygwin64/home/enovid/clips/",
    vcodec = "rawvideo",
    acodec = "pcm_s16le",
    prevf = "",
    ...
    ext = "mkv",
    command_template = [[
        ffmpeg -i "$in" -ss "$start" -to "$end" -c:v copy -c:a copy "$out.$ext"
    ]],
    --command_template = [[
...
function get_homedir()
  ...
  --return "C:\cygwin64\home\enovid\clips"
end
...
function get_outname(shift, endpos)
    ...
    name = name:gsub(" ", "_")	    
	name = "[Clips] " .. name .. string.format(" [%s %s]", timestamp(shift), timestamp(endpos))
    name = name:gsub(":", "-")
    return name
end
...
function cut(shift, endpos)
    ...
    cmd = cmd:gsub("$start", shift)
    cmd = cmd:gsub("$end", endpos)
...
mp.add_key_binding(nil, "clear_mark", clear_mark)
