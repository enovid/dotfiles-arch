...
var zoom_reset_enabled = false;

var path='./modules.js/'
var Options = require(path+'Options'),
    Utils = require(path+'MicroUtils'),
    Ass = require(path+'AssFormat'),
    Stack = require(path+'Stack'),
    RandomCycle = require(path+'RandomCycle');
...
Leapfrog.prototype.toggleZoomReset = function()
{
    if (zoom_reset_enabled === false) {
        zoom_reset_enabled = true;
        mp.osd_message(this._formatMsg('Leapfrog Reset-Zoom: ON'));
    } else  {
        zoom_reset_enabled = false;
        mp.osd_message(this._formatMsg('Leapfrog Reset-Zoom: OFF'));
    }
}
...
Leapfrog.prototype.jump = function(offset, rawOptions)
{
    ...
    // Reset zoom if file is not an image
    var filename = mp.get_property('filename');
    if (zoom_reset_enabled === true)  {
        var ext = filename.slice((filename.lastIndexOf(".") - 1 >>> 0) + 2);
        var zoom_blacklist = ['jpg', 'jpeg', 'png'];
        if (zoom_blacklist.indexOf(ext) == -1)  {
    ...
    if (!options.silent)
        var current_filename = mp.get_property_native('playlist')[newPosition]["filename"]
        current_filename = mp.utils.split_path(current_filename)[1]
        mp.osd_message(
            this._formatMsg(
...
(function() {
    ...
    // After each jump, reset zoom level to default values (0).
    mp.register_script_message('toggle_leapfrog_zoom', function() {
        frog.toggleZoomReset();
    });
})();
