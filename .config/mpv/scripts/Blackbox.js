...
//mp.module_paths.push("./modules")
var path='./modules.js/'
var Options = require(path+'Options'),
    Utils = require(path+'MicroUtils'),
    Ass = require(path+'AssFormat'),
    PathIndex = require(path+'PathIndex'),
    PathTools = require(path+'PathTools'),
    PlaylistManager = require(path+'PlaylistManager'),
    SelectionMenu = require(path+'SelectionMenu');
...
var Blackbox = function(options)
{
    ...
    //this.mediaRgx = /\.(?:mp[234]|m4[av]|mpe?g|m[12]v|web[mp]|mk(?:[av]|3d)|h?264|qt|mov|avi|xvid|divx|wm[av]|asf|pcm|flac|aiff|wav|aac|dts|e?ac3|dat|bin|vob|vcd|mt[sv]|m2ts?|ts|flv|f4[vp]|rm(?:vb)?|3(?:gp|iv)|h?dv|og[gmv])(?:\.part)?$/i;
    //this.mediaRgx = /\.(?:mp[234])(?:\.part)?$/i;
    ...
    try {
        ...
        if (forceRefresh || this.menu.getMetadata().type !== 'files' || this.currentPath !== path) {
            ...
            for (i = 0; i < dirContents.dirs.length; ++i) {
				...
				//mp.msg.error('Path' + dir);
				//mp.osd_message(dir, 2);
				if (dir.split("/").pop() === "trash")
					continue;
