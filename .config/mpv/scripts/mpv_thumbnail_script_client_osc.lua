...
function Thumbnailer:register_client()
    ...
    mp.observe_property("video-dec-params", "native", function()
        ...
        if self.state.available and thumbnailer_options.autogenerate and duration then
...
local osc_styles = {
    ...
    timecodes = "{\\blur0\\bord0\\1c&H000000\\3c&HFFFFFF\\fs20}", -- Progress bar fill
    ...
    smallButtonsBar = "{\\blur0\\bord0\\1c&HBDBDBD\\3c&HFFFFFF\\fs28\\fnmpv-osd-symbols}", -- Play/pause vol fullscreen
    timecodesBar = "{\\blur0\\bord0\\1c&HBDBDBD\\3c&HFFFFFF\\fs27}", -- Time and Progress bg
    timePosBar = "{\\blur0\\bord".. user_opts.tooltipborder .."\\1c&HBDBDBD\\3c&H000000\\fs30}", -- Time on hover
    vidtitleBar = "{\\blur0\\bord0\\1c&HBDBDBD\\3c&HFFFFFF\\fs18\\q2}", -- Video title
}
...
function visibility_mode(mode, no_osd)
    -- toggle between auto and always
    if mode == "toggle" then
        if not state.enabled then
            mode = "auto"
...
function toggle_autogenerate()
    local v = thumbnailer_options.autogenerate 
    if v == true then
        thumbnailer_options.autogenerate = false
        mp.osd_message("Autogenerate off")
    else
        thumbnailer_options.autogenerate = true
        mp.osd_message("Autogenerate on")
    end
end
...
mp.add_key_binding(nil, "visibility-toggle", function() visibility_mode("toggle") end)
mp.add_key_binding(nil, "visibility-never", function() visibility_mode("never") end)
mp.add_key_binding(nil, "toggle_autogenerate", toggle_autogenerate)
